...
class AddAFriendTest < ActionDispatch::IntegrationTest
	...
	test "that adding a friend works"  do
		...
		get "/user_friendships/new?friend_id=#{users(:malina).profile_name}"
		assert_response :success

		assert_difference 'UserFriendship.count' do
			post "/user_friendships",user_friendship: { friend_id: users(:malina).profile_name}
			assert_response :redirect
			assert_equal "You are now friend with #{users(:malina).full_name}"
		end
	end
end
