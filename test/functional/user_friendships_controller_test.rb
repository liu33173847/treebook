...
class UserFriendshipsControllerTest < ActionController::TestCase
	...
	context "when logged in" do
		...
		should "display the friends's name" do
			get :new, friend_id: users(:malina)
			assert_match /#{users(:malina).full_name}/,response.body
		end

		should "assign as new user Friendship" do
			get :new, friend_id: users(:malina)
			assert assigns(:user_friendship)
		end

		should "assign as new user Friendship to the correct friend" do
			get :new, friend_id: users(:malina)
			assert_equal users(:malina), assigns(:user_friendship).friend
		end

		should "assign as new user Friendship to the currect logged in user" do
			get :new, friend_id: users(:malina)
		...
		should "ask if you really want to request this friendship" do
			get :new, friend_id: users(:malina)
			assert_match /Do you really want to friend #{users(:malina).full_name}?/, response.body
		end
	end
			...
			context "successfully" do
				should "create two user friendship objects" do
					assert_difference 'UserFriendship.count', 2 do
						post :create, user_friendship: {friend_id: users(:malina).profile_name}
					end
				end
			end
