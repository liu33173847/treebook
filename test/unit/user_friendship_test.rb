...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	test "that creating a friendship Based on user id and friend id works" do
		UserFriendship.create user_id: users(:jason).id, friend_id: users(:malina).id
		assert users(:jason).pending_friends.include?(users(:malina))
	end

	context "a new instance" do
		setup do
			@user_friendship = UserFriendship.new user: users(:jason), friend: users(:malina)
		end
	...
	context "#send_request_email" do
		setup do
			@user_friendship = UserFriendship.create user: users(:jason), friend: users(:malina)
		end
	...
	context "#mutual_friendship" do
		setup do
			...
			@friendship1 = users(:jason).user_friendships.where(friend_id: users(:malina).id).first
			@friendship2 = users(:malina).user_friendships.where(friend_id: users(:jason).id).first
	...
	context "#accept!" do
		setup do
			@user_friendship = UserFriendship.request users(:jason), users(:malina)
		end
		...
		should "include the friend in the list of friends" do
			...
			assert users(:jason).friends.include?(users(:malina))
		end

		should "accpet the mututal friendship" do
			@user_friendship.accept!
	...
	context "on destory" do
		...
		should "delete the mututal friendship" do
			@friendship1.destroy
			assert !UserFriendship.exists?(@friendship2.id)
		end
	end
